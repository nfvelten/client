...
|---|---:|---|
| Game code | 35,615 lines in 51 files | `git ls-files public/js/*.js \| xargs wc -l` |
...
| Harness scripts | 215 | `git ls-files 'tools/eval/*.mjs' 'tools/eval/*.py' \| wc -l` |
| Pipeline scripts | 59 | `git ls-files 'tools/*.mjs' \| wc -l` |
| Written entry tasks | 26 | `git ls-files 'docs/issues/[0-9]*.md' \| wc -l` |
| Version | `2.0.0-alpha.193` | `public/js/version.js` and `package.json` (match) |
...
| `corrego` | Córrego (Favela de SP) | **capture** | `map_corrego.js` | 1,523 |
...
`package.json` has **132 scripts**; the reason behind each one lives in `SCRIPTS.md`.
